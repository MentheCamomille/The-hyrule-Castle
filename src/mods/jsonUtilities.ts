...
export function getSavedGameSetting() {
  try {
    const file = fs.readFileSync(pathSave, 'utf-8');
...
export function getCharacters(save : boolean) {
  try {
    if (save) {
      ...
      const file = fs.readFileSync(pathCharacters, 'utf-8');
      const characterJson = JSON.parse(file);
...
export function getBosses() {
  try {
    const file = fs.readFileSync(pathBosses, 'utf-8');
...
export function getObject() {
  try{
    const shopInventory = fs.readFileSync(pathObj, 'utf-8');
